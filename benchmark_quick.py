#!/usr/bin/env python3
"""
Quick benchmark script - tests only the fastest models with better progress logging.
"""
...
# Quick test - only test smaller models for speed
MODELS = ['tiny', 'base', 'small']

# Limited settings for quick results
CPU_THREADS = [2, 4, 8]
...
def run_benchmark(audio_file: str) -> List[Dict]:
    ...
    print("=" * 80)
    print("QUICK WHISPER BENCHMARK (Fast Models Only)")
    print("=" * 80)
    print(f"Audio file: {audio_file}")
    print(f"Models to test: {', '.join(MODELS)}")
    print(f"Total configurations: {total_tests}")
    print(f"Estimated time: ~{total_tests * 3}s (assuming 3s per test)")
    print("=" * 80)
    print()
            ...
            for cpu_threads in CPU_THREADS:
                for beam_size in BEAM_SIZES:
                    for vad in VAD_OPTIONS:
                        ...
                        avg_per_test = elapsed / current_test if current_test > 0 else 3
...
def analyze_results(results: List[Dict]) -> None:
    ...
    if sub_1s:
        best = sub_1s[0]
        print("\n" + "-" * 80)
        print("✅ RECOMMENDED CONFIGURATION (fastest sub-1s)")
        print("-" * 80)
...
def save_results(results: List[Dict]) -> None:
    ...
    output_file = f"benchmark_quick_{timestamp}.json"
